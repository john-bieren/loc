...
// languages_info is the map version of languages.json.
var languages_info map[string]map[string]any

// custom_mappings is the map version of the user's custom_mappings.json.
var custom_mappings map[string]map[string]any

// main generates languages.go from scc's languages.json and user's custom_mappings.json.
func main() {
	loadFiles()
	file_lines := generateText()
...
func loadFiles() {
	...
	err = json.Unmarshal(body, &languages_info)
	if err != nil {
	...
	err = decoder.Decode(&custom_mappings)
	if err != nil {
...
func generateText() []string {
	var extra string
	if len(custom_mappings) > 0 {
		extra = " with custom mappings"
	}
	do_not_edit_text := fmt.Sprintf("\n// This file was generated by generator.go%s, DO NOT EDIT IT.\n", extra)
	// file_lines contains the lines of languages.go.
	file_lines := []string{"package main\n", do_not_edit_text}

	// langs_used the languages which appear as values in the extensions and filenames maps.
	langs_used := make(map[string]struct{})
	extension_mappings, filename_mappings, single_char_mappings := gatherLanguageInfo()
	file_lines, langs_used = generateExtensionsMap(file_lines, langs_used, extension_mappings)
	file_lines, langs_used = generateFilenamesMap(file_lines, langs_used, filename_mappings)
	file_lines = generateSingleCharsMap(file_lines, langs_used, single_char_mappings)
	return file_lines
}
...
func gatherLanguageInfo() (map[string]string, map[string]string, map[string][]any) {
	...
	for language, info := range languages_info {
...
func generateExtensionsMap(
	file_lines []string,
	langs_used map[string]struct{},
	extension_mappings map[string]string,
) ([]string, map[string]struct{}) {
	// create union of extension_mappings and custom_mappings, record languages referenced
	custom_extensions, ok := custom_mappings["extensions"]
	if ok {
	...
	for _, extension := range sortKeys(extension_mappings) {
		...
		if language != "" {
			...
			file_lines = append(file_lines, new_line)
			langs_used[language] = struct{}{}
		}
	}
	file_lines = append(file_lines, "\n}\n")
	return file_lines, langs_used
}
...
func generateSingleCharsMap(
	file_lines []string,
	langs_used map[string]struct{},
	single_char_mappings map[string][]any,
) []string {
	// create union of single_char_mappings and custom_single_chars
	custom_single_chars, ok := custom_mappings["single_line_comment_chars"]
	if ok {
	...
	for _, language := range sortKeys(single_char_mappings) {
		...
		if _, ok := langs_used[language]; !ok {
			continue
		}
		...
		if len(char_set) > 0 {
			...
			file_lines = append(file_lines, fmt.Sprintf("\n\t\"%s\": {},", language))
		}
	}
	file_lines = append(file_lines, "\n}\n")
	return file_lines
